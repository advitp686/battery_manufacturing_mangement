...
function openModal(kind) {
  ...
  if (kind === 'purchase-bill') {
    ...
    bindVehicleLines(); bind($('.purchase-line', lines)); toggleVehicleFields(false); $('#btn-add-purchase-line').addEventListener('click',()=>{lines.insertAdjacentHTML('beforeend',makeRow());bind(lines.lastElementChild);recalc();}); applyPurchaseTaxMode(); setTimeout(() => { const section = $('#purchase-item-section'); if (section) section.scrollLeft = 0; }, 0);
...
function getPurchaseBillTaxableValue(bill) {
  const stored = Number(bill?.taxableValue || 0);
  if (stored > 0) return stored;
  const itemTotal = (bill?.items || []).reduce((sum, item) => sum + (Number(item.taxableValue || 0) || Number(item.qty || 1) * Number(item.unitPrice ?? item.price ?? 0)), 0);
  if (itemTotal > 0) return itemTotal;
  const total = Number(bill?.grandTotal || 0);
  const tax = Number(bill?.cgstAmount || 0) + Number(bill?.sgstAmount || 0) + Number(bill?.igstAmount || 0) + Number(bill?.otherAmount || 0);
  return Math.max(0, total - tax - Number(bill?.vehicleOtherCharges || 0));
}

function getPurchaseBillBalance(bill) {
  const grandTotal = Number(bill?.grandTotal || 0);
  const paid = Number(bill?.paidAmount || 0);
  const stored = bill?.balanceAmount;
  if (stored == null || (Number(stored) === 0 && paid < grandTotal)) return Math.max(0, grandTotal - paid);
