...
function getPurchaseTaxModeFromGstin(vendorGstin = '') {
  const companyCode = getGstinStateCode(getSystemSettings().gstin);
  const vendorCode = getGstinStateCode(vendorGstin);
  if (!companyCode || !vendorCode) return '';
...
function openModal(kind) {
  ...
  if (kind === 'purchase-bill') {
    ...
    const fillVendorGstin = () => { const name=normalizeText($('#purchase-supplier').value); const match=[...(state.suppliers||[]), ...(state.dealers||[])].find(s=>normalizeText(s.name)===name); if(match?.gstin && !$('#purchase-vendor-gstin').value) $('#purchase-vendor-gstin').value=match.gstin; applyPurchaseTaxMode(); };
    ...
    $('#purchase-supplier').addEventListener('change', fillVendorGstin); $('#purchase-vendor-gstin').addEventListener('input', applyPurchaseTaxMode); $('#purchase-payment-percent').addEventListener('input', recalc); $('#purchase-payment-status').addEventListener('change', e => { $('#purchase-payment-percent').value=e.target.value==='Paid'?'100':'25'; recalc(); }); $('#purchase-type').addEventListener('change', e => { const isVehicle=e.target.value==='vehicle'; $('#vehicle-purchase-fields').hidden=!isVehicle; $('#purchase-item-section').hidden=isVehicle; toggleVehicleFields(isVehicle); recalc(); }); $('#btn-add-vehicle-line').addEventListener('click',()=>{ $('#vehicle-purchase-lines').insertAdjacentHTML('beforeend',vehicleEntry); bindVehicleLines(); toggleVehicleFields($('#purchase-type').value==='vehicle'); recalc(); });
    const bindVehicleLines = () => { $$('.vehicle-purchase-entry').forEach(v=>{ v.oninput=recalc; v.querySelector('.btn-remove-vehicle-line').onclick=()=>{if($$('.vehicle-purchase-entry').length>1){v.remove();recalc();}}; }); };
    bindVehicleLines(); bind($('.purchase-line', lines)); toggleVehicleFields(false); $('#btn-add-purchase-line').addEventListener('click',()=>{lines.insertAdjacentHTML('beforeend',makeRow());bind(lines.lastElementChild);recalc();}); applyPurchaseTaxMode();
...
async function submitModal(e) {
  ...
  if (kind === 'purchase-bill') {
    ...
    const taxableValue=items.reduce((s,x)=>s+x.taxableValue,0),cgstAmount=items.reduce((s,x)=>s+x.cgstAmount,0),sgstAmount=items.reduce((s,x)=>s+x.sgstAmount,0),igstAmount=items.reduce((s,x)=>s+x.igstAmount,0),otherAmount=items.reduce((s,x)=>s+x.otherAmount,0),vehicleOtherCharges=data.purchase_type==='vehicle'?vehiclePurchases.reduce((s,v)=>s+v.otherCharges,0):0,grandTotal=taxableValue+cgstAmount+sgstAmount+igstAmount+otherAmount,date=data.billDate||new Date().toISOString().slice(0,10),paymentPercent=Math.max(0,Math.min(100,Number(data.payment_status==='Paid'?100:data.payment_percent||0))),paidAmount=grandTotal*paymentPercent/100,balanceAmount=grandTotal-paidAmount,paymentMode=data.payment_mode||'HDFC Bank Current A/C (50200012345678)',billId='PB-'+Date.now();
...
function printPurchaseBill(billNo) {
  const bill = (state.purchaseBills || []).find(item => item.billNo === billNo);
  if (!bill) return;
  const settings = getSystemSettings();
  const supplier = (state.suppliers || []).find(s => normalizeText(s.name) === normalizeText(bill.supplier)) || {};
  const totalGst = Number(bill.cgstAmount || 0) + Number(bill.sgstAmount || 0) + Number(bill.igstAmount || 0);
  const backdrop = $('#modal-backdrop');
  if (!backdrop) return;
  $('#modal-title').textContent = `Purchase Invoice — ${bill.billNo}`;
  $('.modal').style.width = 'min(900px, 98%)';
  $('#modal-fields').innerHTML = `<div class="purchase-invoice-printable" style="font-family:'Segoe UI',sans-serif;color:#000;background:#fff;padding:20px;border:1px solid #000;"><div style="text-align:center;border-bottom:1px solid #000;padding-bottom:10px;"><h1 style="margin:0;font-size:23px;">${settings.companyName || 'Lithynova EV Battery Systems'}</h1><div>${settings.address || ''}</div><div>GSTIN: ${settings.gstin || '—'}</div></div><div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #000;font-size:12px;"><div><strong>SUPPLIER / VENDOR</strong><br>${bill.supplier}<br>${supplier.address || ''}<br>GSTIN: ${bill.vendorGstin || supplier.gstin || 'Unregistered'}</div><div><strong>PURCHASE BILL</strong><br>Bill No: ${bill.billNo}<br>Date: ${bill.billDate}<br>Tax mode: ${bill.taxMode || 'INTRA'}</div></div><table style="width:100%;border-collapse:collapse;margin-top:12px;font-size:11px;"><thead><tr style="background:#f1f5f9;"><th style="border:1px solid #000;padding:6px;">#</th><th style="border:1px solid #000;padding:6px;text-align:left;">Description</th><th style="border:1px solid #000;padding:6px;">HSN</th><th style="border:1px solid #000;padding:6px;">Qty</th><th style="border:1px solid #000;padding:6px;text-align:right;">Rate</th><th style="border:1px solid #000;padding:6px;text-align:right;">Taxable</th><th style="border:1px solid #000;padding:6px;text-align:right;">GST</th><th style="border:1px solid #000;padding:6px;text-align:right;">Amount</th></tr></thead><tbody>${(bill.items || []).map((item, i) => { const gst=Number(item.cgstAmount||0)+Number(item.sgstAmount||0)+Number(item.igstAmount||0)+Number(item.otherAmount||0); const amount=Number(item.taxableValue||0)+gst; return `<tr><td style="border:1px solid #000;padding:6px;text-align:center;">${i+1}</td><td style="border:1px solid #000;padding:6px;">${item.name || item.modelNo || 'Item'}</td><td style="border:1px solid #000;padding:6px;text-align:center;">${item.hsn || '—'}</td><td style="border:1px solid #000;padding:6px;text-align:center;">${item.qty || 1}</td><td style="border:1px solid #000;padding:6px;text-align:right;">${Number(item.unitPrice || item.price || 0).toFixed(2)}</td><td style="border:1px solid #000;padding:6px;text-align:right;">${Number(item.taxableValue || 0).toFixed(2)}</td><td style="border:1px solid #000;padding:6px;text-align:right;">${gst.toFixed(2)}</td><td style="border:1px solid #000;padding:6px;text-align:right;">${amount.toFixed(2)}</td></tr>`; }).join('')}</tbody></table><div style="display:flex;justify-content:flex-end;margin-top:12px;"><div style="width:280px;font-size:12px;"><div style="display:flex;justify-content:space-between;">Taxable value <strong>${Number(bill.taxableValue || 0).toFixed(2)}</strong></div><div style="display:flex;justify-content:space-between;">CGST + SGST <strong>${(Number(bill.cgstAmount||0)+Number(bill.sgstAmount||0)).toFixed(2)}</strong></div><div style="display:flex;justify-content:space-between;">IGST / Other <strong>${(Number(bill.igstAmount||0)+Number(bill.otherAmount||0)).toFixed(2)}</strong></div><div style="display:flex;justify-content:space-between;border-top:1px solid #000;padding-top:6px;font-size:14px;">Grand total <strong>₹${Number(bill.grandTotal || 0).toFixed(2)}</strong></div><div style="display:flex;justify-content:space-between;color:#2f855a;">Paid (${Number(bill.paymentPercent || 0)}%) <strong>₹${Number(bill.paidAmount || 0).toFixed(2)}</strong></div><div style="display:flex;justify-content:space-between;color:#c53030;">Balance payable <strong>₹${Number(bill.balanceAmount ?? bill.grandTotal ?? 0).toFixed(2)}</strong></div></div></div><div style="margin-top:18px;font-size:11px;border-top:1px solid #000;padding-top:8px;">Payment terms: ${bill.paymentStatus === 'Paid' ? 'Fully paid' : `Credit purchase — ${Number(bill.paymentPercent || 0)}% paid now`}. Payment mode: ${bill.paymentMode || '—'}</div></div><div style="text-align:center;margin-top:10px;"><button type="button" class="primary-btn" onclick="window.print()">🖨️ Print Purchase Invoice</button></div>`;
...
function bind() {
  ...
  document.addEventListener('click', (e) => {
    ...
    const printPurchaseBtn = e.target.closest('.btn-print-purchase-bill');
    if (printPurchaseBtn) {
      e.preventDefault();
