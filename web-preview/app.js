...
function openModal(kind) {
  ...
  if (kind === 'purchase-bill') {
    ...
    $('#purchase-supplier').addEventListener('change', fillVendorGstin); $('#purchase-vendor-gstin').addEventListener('input', applyPurchaseTaxMode); $('#purchase-type').addEventListener('change', e => { $('#vehicle-purchase-fields').hidden=e.target.value!=='vehicle'; recalc(); });
    bind($('.purchase-line', lines)); $('#btn-add-purchase-line').addEventListener('click',()=>{lines.insertAdjacentHTML('beforeend',makeRow());bind(lines.lastElementChild);recalc();}); applyPurchaseTaxMode();
