...
function openModal(kind) {
  ...
  if (kind === 'purchase-bill') {
    ...
    $('#purchase-supplier').addEventListener('change', fillVendorGstin); $('#purchase-vendor-gstin').addEventListener('input', applyPurchaseTaxMode); $('#purchase-type').addEventListener('change', e => { const isVehicle=e.target.value==='vehicle'; $('#vehicle-purchase-fields').hidden=!isVehicle; $('#purchase-item-section').hidden=isVehicle; recalc(); }); $('#btn-add-vehicle-line').addEventListener('click',()=>{ $('#vehicle-purchase-lines').insertAdjacentHTML('beforeend',vehicleEntry); bindVehicleLines(); recalc(); });
    ...
    bindVehicleLines(); bind($('.purchase-line', lines)); $('#btn-add-purchase-line').addEventListener('click',()=>{lines.insertAdjacentHTML('beforeend',makeRow());bind(lines.lastElementChild);recalc();}); applyPurchaseTaxMode();
...
async function submitModal(e) {
  ...
  if (kind === 'purchase-bill') {
    ...
    const taxableValue=items.reduce((s,x)=>s+x.taxableValue,0),cgstAmount=items.reduce((s,x)=>s+x.cgstAmount,0),sgstAmount=items.reduce((s,x)=>s+x.sgstAmount,0),igstAmount=items.reduce((s,x)=>s+x.igstAmount,0),otherAmount=items.reduce((s,x)=>s+x.otherAmount,0),vehicleOtherCharges=data.purchase_type==='vehicle'?vehiclePurchases.reduce((s,v)=>s+v.otherCharges,0):0,grandTotal=taxableValue+cgstAmount+sgstAmount+igstAmount+otherAmount,date=data.billDate||new Date().toISOString().slice(0,10),bankAccount='HDFC Bank Current A/C (50200012345678)',billId='PB-'+Date.now();
    ...
    if(!state.purchaseBills)state.purchaseBills=[]; state.purchaseBills.unshift({id:billId,billNo:data.billNo,billDate:date,ewayBillNo:data.ewayBillNo||'',supplier:data.supplier,vendorGstin,taxMode,cgstAmount,sgstAmount,igstAmount,otherAmount,vehicleOtherCharges,grandTotal,items});
